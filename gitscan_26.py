...
def process_repo(repo, token, all_results, domain, passwords):
    ...
    for commit in commits:
        ...
        if file_content:
            confidence, matched_pattern = classify_match(file_content)
            if confidence:
                ...
                all_results.append([
                    ...
                    "Sensitive Match", matched_pattern, "REDACTED", last_modified, file_type, confidence
                ])
...
def main():
    ...
    with open(password_file, 'r') as file:
        passwords = [line.strip().lower() for line in file]
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        for repo in repos:
            executor.submit(process_repo, repo, token, all_results, domain, passwords)
